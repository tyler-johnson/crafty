...
Environment.prototype.mergeRuntime = function() {
	...
	return Promise.props(promises)
		.bind(this)
		.then(function(props) {
			var promises = _.map(props, function(data, key) {
				...
				if (_.isArray(data)) {
					...
					self.prop(key, data, false);
				}

				var d = self.prop(key);
				if (!_.isEqual(d, data)) return self._writeFile(RUNTIME_FILES[key], d);
			});

			promises.push(self._writeProps());
